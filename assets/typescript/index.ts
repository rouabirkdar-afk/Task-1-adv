...
class Book {
    ...
    displayinfo(title: string, author: string, category: string, isAvailable: boolean): string {
        if (this.#isAvailable) {
...
class ReferenceBook extends Book {
    ...
    displayinfo(title: string, author: string, category: string, isAvailable: boolean, locationCode?: string): string {
        if (isAvailable) {
...
class Library {
    ...
    filterByCategory(select: string): Book[] {
        if (select == "ALL") {
            return this.#books;
        }
        return this.#books.filter(book => book.getCategory() == select)
    }
    toggleIsAvailabiaity(title: string): void {
...
const searchBooks = document.getElementById("bookSearch") as HTMLInputElement;
const categoryOpthins = document.querySelectorAll('#Catgories li');

function render(booksArray: Book[]) {
...
categoryOpthins.forEach(option => {
    option.addEventListener('click', (e) => {
        const categorySelect = (e.target as HTMLElement).getAttribute('data-value');
        if (categorySelect) {
...
addBtn?.addEventListener("click", () => {
    ...
    const cat = (document.getElementById("newCategory") as HTMLSelectElement).value;
